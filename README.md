...
Alternatively, download the latest release from https://github.com/GinjaChris/OpenDDOS/releases, extract it and run the script.
Or use git clone:

```
git clone https://github.com/GinjaChris/OpenDDOS
```
...
* Listener - uses netcat to open a listener on a configurable TCP or UDP port.  This can be useful for testing syslog connectivity, receive files or checking for active scanning on the network. Anything received by the listener is written out to ./OpenDDOS.listener.out or a file of your choice.
...
Pentmenu is available as a [package](https://archstrike.org/packages/OpenDDOS) on Arch Linux. Big love to [ArchStrike](https://archstrike.org/) and [Parrot linux](https://www.parrotsec.org/).
